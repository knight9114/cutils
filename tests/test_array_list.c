...
void test_array_list_find(void) {
  printf("testing array_list_find ... ");
  ...
  size_t idx = SIZE_MAX;
  err = array_list_find(l, v1, cmp, &idx);
  assert(err == CUTILS_SUCCESS);
  assert(idx == 0);

  idx = SIZE_MAX;
  err = array_list_find(l, v2, cmp, &idx);
  assert(err == CUTILS_SUCCESS);
  assert(idx == 1);

  idx = SIZE_MAX;
  err = array_list_find(l, v3, cmp, &idx);
  assert(err == CUTILS_SUCCESS);
  assert(idx == 2);

  test_data_t *v4 = _new(1);
  idx = SIZE_MAX;
  err = array_list_find(l, v4, cmp, &idx);
  assert(err == CUTILS_SUCCESS);
  assert(idx == SIZE_MAX);
